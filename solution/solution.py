...
def call_openai_mini(
    ...
    return call_openai(model=OPENAI_MINI_MODEL, prompt=prompt)
...
def compare_models(prompt: str) -> dict:
    ...
    text, latency = call_openai(prompt=prompt)
    text_m, latency_m = call_openai_mini(prompt=prompt)
    
    cost = (len(text.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    
    return {
        "gpt4o_response": text,
        "mini_response": text_m,
        "gpt4o_latency": latency,
        "mini_latency": latency_m,
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    input_cost = input_token / 1000 * pricing["input"]
    output_cost = output_token / 1000 * pricing["output"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        row = []
        for key in ("prompt", "gpt4o_response", "mini_response"):
            text = " ".join(result[key].split())
            row.append(text if len(text) <= 40 else text[:37] + "...")
        row.extend([
            f"{result['gpt4o_latency']:.3f}s",
            f"{result['mini_latency']:.3f}s",
        ])
        rows.append(row)
